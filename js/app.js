...
function renderAnalytics() {
  ...
  drawBarChart("chart-revenue", revData.map((d) => d.name), revData.map((d) => +(d.rev / 10000000).toFixed(2)), ["#3b82f6", "#f59e0b", "#10b981", "#8b5cf6", "#ef4444", "#06b6d4", "#f97316"]);

  const sc = { Completed: 0, Confirmed: 0, Pending: 0, Cancelled: 0 };
  DB.bookings.forEach((b) => sc[b.status] = (sc[b.status] || 0) + 1);
  drawPieChart("chart-booking-status", Object.keys(sc), Object.values(sc), ["#10b981", "#3b82f6", "#f59e0b", "#ef4444"]);
  ...
  drawBarChart("chart-locations", locData.map((d) => d.name), locData.map((d) => +(d.rev / 10000000).toFixed(2)), ["#8b5cf6", "#3b82f6", "#10b981", "#f59e0b", "#ef4444", "#06b6d4"]);
...
function drawBarChart(canvasId, labels, values, colors) {
  ...
  if (!values.length) { ctx.fillStyle = "#475569"; ctx.font = "13px DM Sans"; ctx.fillText("No data available", W / 2 - 60, H / 2); return; }
  const max = Math.max(...values) * 1.2 || 1;
  const barW = Math.min(50, (W - 60) / values.length - 10);
  const gap = (W - 60 - barW * values.length) / (values.length + 1);
  values.forEach((v, i) => {
    const x = 40 + gap * (i + 1) + barW * i;
    ...
    ctx.fill();
    ctx.fillStyle = "#f1f5f9"; ctx.font = "bold 11px DM Sans"; ctx.textAlign = "center";
    ctx.fillText(v + " Cr", x + barW / 2, y - 5);
    ctx.fillStyle = "#94a3b8"; ctx.font = "10px DM Sans";
    const lbl = labels[i].length > 9 ? labels[i].substring(0, 9) + "..." : labels[i];
...
function drawPieChart(canvasId, labels, values, colors) {
  ...
  if (!total) { ctx.fillStyle = "#475569"; ctx.font = "13px DM Sans"; ctx.textAlign = "center"; ctx.fillText("No data", W / 2, H / 2); return; }
  ...
  labels.forEach((l, i) => {
    const ly = 30 + i * 38;
    ctx.fillStyle = colors[i]; ctx.fillRect(W * 0.7, ly, 12, 12);
    ctx.fillStyle = "#94a3b8"; ctx.font = "11px DM Sans"; ctx.textAlign = "left";
    ctx.fillText(l, W * 0.7 + 16, ly + 10);
    ctx.fillStyle = "#f1f5f9"; ctx.font = "bold 11px DM Sans";
    ctx.fillText(Math.round(values[i] / total * 100) + "%", W * 0.7 + 16, ly + 22);
  });
}
